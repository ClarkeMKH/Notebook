function Allow-Taskbar-Pinning {
    function global:Set-TaskbarPin {
        # Version 1.4
        <#
            .EXAMPLE
...
# ==================== SAVING AND LOADING CUSTOM DATA TO LOCAL FILE ====================

# ========== Create File ==========
$script:DataFile = Join-Path $PSScriptRoot "Companion Data.json"
if (-not (Test-Path $script:DataFile)) {
    '{"Version": 1}' | Set-Content -Path $script:DataFile -Encoding UTF8
}
function Get-Data {
...
function Elevate-Admin {
    if (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
        ...
        Write-Host "You are already running as Admin."
    }
}
...
function Sign-In-Microsoft {
    if (-not (Get-MgContext)) {
        ...
        Write-Host "`nMicrosoft Sign In" -ForegroundColor Cyan
        Write-Host "Go to: $($device.verification_uri)"
        Write-Host "Enter code: $($device.user_code)`n"
        $tokenBody = @{ grant_type = "urn:ietf:params:oauth:grant-type:device_code"; client_id = $clientId; device_code = $device.device_code }
        $interval = [Math]::Max($device.interval, 5)
        while ($true) {
            Start-Sleep -Seconds $interval
            try {
                $token = Invoke-RestMethod -Method Post -Uri "https://login.microsoftonline.com/$script:tenant/oauth2/v2.0/token" -Body $tokenBody -ErrorAction Stop
                break
            } catch {
                ...
                switch ($err.error) {
                    ...
                    default {
                        ...
                        Write-Host "Sign-in failed: $msg" -ForegroundColor Red
                        return
                    }
                }
            }
    ...
    Show-Menu -Text "Microsoft 365 : $((Get-MgContext).Account)" -Options ([ordered]@{
...
function Sign-Out-Microsoft {
    if (Get-MgContext) {
        Write-Host "Signing out of $((Get-MgContext).Account)." -ForegroundColor Yellow
...
<body sytle="margin:0px; padding:0px; width:100%; height:100dvh;">
    <div id="header" style="width:100%; height:120px; display:flex; align-items:center; position:fixed; left:0px; top:0px; border-bottom:solid #dddddd 1px; background-color:#FAF3E3;">
        <img src="./logo.png" style="height:80px; width:80px; margin:9px 20px; outline:none; border:solid 1px #ddddcc; border-radius:100px; box-shadow:1px 1px 2px #999999; cursor:pointer;"></img>
        
        <div style="width:100%; font-size:20px; font-weight:600;">Click and Treat Training</div>
        <div style="min-width:50px; max-width:50px; width:50px; height:50px; border:solid 1px #ddddcc; border-radius:12px; box-shadow:1px 1px 2px #999999; margin:0px 40px 0px 20px; background-color:#ffffff; cursor:pointer;"></div>
    </div>
...
"@
    ...
    $listener.Start()
    Write-Host "Listening on http://localhost:8080/ (Ctrl+C to stop)"
    Start-Process $url
...
function Setup-Machine {
    # ========== AUTH KEY FOR MY TAILSCALE ADMIN ACCOUNT ==========
    $tailscaleAdminAuthKey = ""

    # ========== ELEVATION CHECK ==========
    if (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
        Write-Host "You must elevate this to administrator privileges to run this command." -ForegroundColor Yellow
        return
    }

    # ========== SETUP LOCAL ADMIN ==========
    $username = "Clarke-Agent-Profile" 
    New-LocalUser -Name $username -Password (ConvertTo-SecureString "Password" -AsPlainText -Force) -PasswordNeverExpires -UserMayNotChangePassword -AccountNeverExpires -Description "Dedicated account for Clarkes remote agent."
    ...
    # ========== INSTALL / SETUP TAILSCALE AND OPENSHH ==========
    ...
    & $tailscaleExe up --authkey=$tailscaleAdminAuthKey --unattended
    & $tailscaleExe status
    ...
    if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
        ...
        # Refresh PATH so 'choco' is recognized in this session
        $env:Path = [System.Environment]::GetEnvironmentVariable("Path","Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path","User")
    }

    Write-Host "Beginning Installtion of Applications."

    # ========== INSTALL CHROME ==========
    # choco install googlechrome -y --ignore-checksums *> $null

    # ========== INSTALL DELLCOMMANDUPDATE IF DELL ===========
    if ((Get-ComputerInfo).CsManufacturer -eq "Dell Inc.") {
        choco install dellcommandupdate -y *> $null
        Start-Sleep -Seconds 1
        Start-Process "C:\Program Files (x86)\Dell\CommandUpdate\DellCommandUpdate.exe"
    }

    # ========== INSTALL VLC MEDIA PLAYER ==========
    choco install vlc -y *> $null

    # ========== INSTALL BAMBU STUDIO ==========
    choco install bambustudio -y --version=2.4.0.70 *> $null

    # ========== INSTALL STEAM ==========
    choco install steam -y *> $null

    # ========== INSTALL WINRAR ==========
    choco install winrar -y *> $null

    # ========== INSTALL VS CODE ==========
    choco install vscode -y *> $null

    
    Write-Host "Applications Installed.`n`nOrganizing Taskbar."

    # ========== PIN APPS ==========
    Set-TaskbarPin -Unpin * -Silent
    Start-Sleep -Seconds 2
    Set-TaskbarPin "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" -Silent
    Start-Sleep -Seconds 1
    Set-TaskbarPin "C:\Program Files\Google\Chrome\Application\chrome.exe" -Silent
    Start-Sleep -Seconds 1
    Set-TaskbarPin "shell:AppsFolder\Microsoft.Windows.Explorer" -Silent
    Start-Sleep -Seconds 1
    Set-TaskbarPin "C:\Program Files\Bambu Studio\bambu-studio.exe" -Silent
    # Start-Sleep -Seconds 1
    # Set-TaskbarPin "C:\Program Files\Microsoft Office\root\Office16\OUTLOOK.EXE" -Silent
    Start-Sleep -Seconds 1
    $Apps = (New-Object -ComObject Shell.Application).NameSpace('shell:AppsFolder').Items()
    $Result = $Apps | Where-Object { $_.name -eq "Outlook" } | Select-Object name,@{n="AUMID";e={$_.path}}
    Set-TaskbarPin ("shell:AppsFolder\" + $Result.AUMID) -Silent
    Start-Sleep -Seconds 1
    Set-TaskbarPin "C:\Program Files\Microsoft Office\root\Office16\WINWORD.EXE" -Silent
    Start-Sleep -Seconds 1
    Set-TaskbarPin "C:\Program Files\Microsoft Office\root\Office16\EXCEL.EXE" -Silent
    # Start-Sleep -Seconds 1
    # Set-TaskbarPin "C:\Program Files\Microsoft Office\root\Office16\POWERPNT.EXE" -Silent
    Start-Sleep -Seconds 1

    # ========== ENABLE BITLOCKER ==========
    $status = (Get-BitLockerVolume -MountPoint "C:").ProtectionStatus
    if ($status -ne 'On') {
        Enable-BitLocker -MountPoint "C:" -RecoveryPasswordProtector -SkipHardwareTest *> $null
    }
    
    # ========== GET BITLOCKER KEY ==========
    if ($status -eq 'On') {
        $key = (Get-BitLockerVolume -MountPoint "C:").KeyProtector | Where-Object { $_.KeyProtectorType -eq 'RecoveryPassword' }
        $key.RecoveryPassword > "$([Environment]::GetFolderPath('Desktop'))\bitlocker-key.txt"
    }

    # ========== SETTING WALLPAPER ==========
    $ProgressPreference = 'SilentlyContinue'
    Invoke-WebRequest "https://clarkemkh.github.io/Notebook/Wallpaper.jpg" -OutFile "$env:TEMP\wallpaper.jpg"
    Set-ItemProperty 'HKCU:\Control Panel\Desktop' Wallpaper "$env:TEMP\wallpaper.jpg"
    rundll32.exe user32.dll,UpdatePerUserSystemParameters

    # ========== FINISHED ==========
    Write-Host "`nMachine Setup Complete."
}



# ==================== CONVERT ANY CODE INTO AN EXE ====================

function Export-PS-To-Exe {
    param (
        [string]$Code,
        [string]$FilePath = "$([Environment]::GetFolderPath('Desktop'))\Terminal-Companion-Executable.exe",
        [switch]$Admin
    )
    ...
    $ScriptPath = Join-Path $env:TEMP "temp-script.ps1"
    Set-Content -Path $ScriptPath -Value $Code
    $ps2exeParams = @{inputFile  = $ScriptPath; outputFile = $FilePath; noConsole  = $true}
    if ($Admin) {
    ...
    Write-Host "Executable Created : $FilePath"
}
...
# ==================== CHECKS N8N IF OUT OF DATE AND DOWNLOADS UPDATED IF SO ====================

function Update-From-N8N {
    $response = Invoke-RestMethod -Uri "https://automation.harrison-home.co.uk/webhook/d8a4d3ae-8d71-4316-af21-70e6ffc9e427" -Method Get -Body @{Version = (Load-Data -Key "Version")}
    if ($response) {
        Write-Host "Out of date."
        Save-Data -Key "Version" -Value $response.Version
        $currentPath  = $PSCommandPath
        $previousPath = Join-Path $PSScriptRoot "Terminal-Companion-Previous.ps1"
        if (Test-Path $previousPath) {
            Remove-Item -Path $previousPath -Force
        }
        Rename-Item -Path $currentPath -NewName "Terminal-Companion-Previous.ps1"
        $response.Code | Set-Content -Path $currentPath
        Write-Host "Downloaded up to date version of Terminal Companion"
    }
    else {
...
# ==================== UPDATE THE VERSION + CODE IN N8N ====================

function Update-Script-In-N8N {
    $version = (Load-Data -Key "Version") + 1
    Save-Data -Key "Version" -Value $version
    $response = Invoke-RestMethod -Uri "https://automation.harrison-home.co.uk/webhook/95dcf92c-42f2-46fe-ae63-8573696e9d85" -Method Post -Body (@{Version = $version; Code = (Get-Content -Path $PSCommandPath -Raw)} | ConvertTo-Json) -ContentType "application/json"
    Clear-Host
    Read-Host "Successfully updated N8N.`n`nPress any button to return"
    Show-Menu
}
...
function Show-Menu {
    param(
        [string]$Text = "Hi, Welcome to the Terminal Companion created by Clarke Harrison`n`nPlease select from one of the options below.",
        [object]$Options = ([ordered]@{
            "Make Admin" = { Elevate-Admin }
            "Microsoft 365" = { Sign-In-Microsoft }
            "Host Website Builder" = { Host-WebsiteBuilder }
            "Connect to Tailscale" = { Write-Host "In Development" }
            "Setup PC" = { Write-Host "Temporarily Disabled" }#Setup-Machine }
            "Create PC Setup Exe" = { Write-Host "Temporarily Disabled" } # Export-PS-To-Exe -Code ((Get-Command Allow-Taskbar-Pinning).Definition + "`n`n" + (Get-Command Setup-Machine).Definition) -FilePath "$([Environment]::GetFolderPath('Desktop'))\Setup-PC.exe" -Admin }
            "Update from N8N" = { Update-From-N8N }
            "Update to N8N" = { Update-Script-In-N8N }
    ...
    foreach ($key in $Options.Keys) {
        ...
        if ($i -eq $Selected) {
            Write-Host " $key " -ForegroundColor Black -BackgroundColor White
        }
        else {
            Write-Host " $key "
        }
    ...
    while ($Options) {
        ...
        switch ($key.Key) {
            'UpArrow'   { Show-Menu -Text $Text -Options $Options -Selected (($Selected - 1 + $Options.Count) % $Options.Count) }
            'DownArrow' { Show-Menu -Text $Text -Options $Options -Selected (($Selected + 1) % $Options.Count) }
            'Enter'     {
                & @($Options.GetEnumerator())[$Selected].Value
                return
            }
        }
    }
