...
function Save-Data {
    ...
    $DataFile = Join-Path $PSScriptRoot "Companion Data.json"
...
function Install-App {
    ...
    if ($Choco) {
        if (-NOT (Get-Admin)) { Set-Admin }
        if (-NOT (Get-App -Name "choco")) {
...
function Uninstall-App {
    ...
    if ($Choco) {
        if (-NOT (Get-Admin)) { Set-Admin }
        choco uninstall $Name -y --ignore-checksums
    }
}
...
function Webhook {
    ...
    try {
        return Invoke-RestMethod -Uri $Url -Method $Type -Body ($Body | ConvertTo-Json -Depth 10) -ContentType 'application/json' -TimeoutSec 5
    } catch {
    ...
    try {
        if ($Body) {
            #return Invoke-RestMethod -Uri $Url -Method $Type -Body ($Body | ConvertTo-Json -Depth 10) -ContentType 'application/json'
        } else {
            #return Invoke-RestMethod -Uri $Url -Method $Type
        }
    } catch {
...
'@
    ...
    Set-Content $tmp "$RunAsAdmin`n`n$Code"
    $args = @{ inputFile = $tmp; outputFile = $Path }
    if ($IconPath -and (Test-Path $IconPath)) { $args.iconFile = $IconPath }
    if (-not $ShowConsole) { $args.noConsole = $true }
    Invoke-ps2exe @args *> $null
    Remove-Item $tmp
...
function Install-MeshAgent {
    $AgentUrl = 'https://support.harrison-home.co.uk/meshagents?id=4&meshid=xQdDMbgGGPr$qZcaxWoGRimYIQTXuN1hoD40rUDv@PgW4Wyo$PNcNBIklXVSIBhd&installflags=0'
    $exePath = "$env:TEMP\meshagent.exe"
    Invoke-WebRequest -Uri $AgentUrl -OutFile $exePath -UseBasicParsing
    Start-Process -FilePath $exePath -ArgumentList "-fullinstall" -Wait -WindowStyle Hidden *> $null
}
...
function Show-Menu {
    ...
    if (-NOT $Options) {
         $Options = [ordered]@{
            "Create Agent Executable" = { Export-Code -Code ( (Get-Command Install-MeshAgent).Definition) -Path "C:\Users\Laptop\OneDrive - KentCo\Desktop\Agent.exe" -IconPath "C:\Users\Laptop\Downloads\logo.ico" }
            "Update to N8N" = { Update-Script-In-N8N } # WebHook -Url 'https://automation.harrison-home.co.uk/webhook/95dcf92c-42f2-46fe-ae63-8573696e9d85' -Type 'Post' -Body (@{Code = (Get-Content -Path $PSCommandPath -Raw)}); Write-Host "N8N Successfully Updated!"; }
            "========== ADMIN TOOLS ==========" = { Show-Menu -Selected $Selected }
            "Grant Admin" = { Open-Admin-Companion }
        }
        if (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
            $Options.Remove("Grant Admin")
            $Options["Direct Mesh Install"] = { Install-MeshAgent }
        }
    }

    Clear-Host
    Write-Host "================================================================`n                      TERMINAL COMPANION                      `n================================================================`n`n"
    Write-Host "$Text`n`n`n"
    $i = 0
    foreach ($key in $Options.Keys) {
        Write-Host "      " -NoNewline
        if ($i -eq $Selected) {
            Write-Host " $key `n" -ForegroundColor Black -BackgroundColor White
        }
        else {
            Write-Host " $key `n"
        }
