...
function Show-Menu {
    param(
        ...
        [object]$Options = ([ordered]@{
            "Microsoft 365" = { Write-Host "Microsoft" }
...
function Setup-Machine {
    ...
    if (-not (Test-Path "$env:ProgramFiles\Bambu Studio\bambu-studio.exe")) {
        Write-Host "Bambu Studio not found. Installing..."
        choco install bambustudio -y --version=2.4.0.70 --no-progress
    } else {
        Write-Host "Bambu Studio is already installed."
    }
    ...
    # ========== PIN APPS ==========
    Set-TaskbarPin -Unpin * -AllUsers -Silent
