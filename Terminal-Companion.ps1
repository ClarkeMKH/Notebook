...
function Host-WebsiteBuilder {
    ...
    <body><h1>Hello from PowerShell!</h1></body>
    </html>
"@
...
function Setup-Machine {
    ...
    choco install bambustudio -y --version=2.4.0.70
    ...
    # ========== INSTALL CURSEFORGE ==========
    if (-not (Test-Path "$env:LOCALAPPDATA\Programs\CurseForge Windows\CurseForge.exe")) {
        Write-Host "CurseForge not found. Installing..."
        winget install -e --id Overwolf.CurseForge --silent --accept-package-agreements --accept-source-agreements
    } else {
        Write-Host "CurseForge is already installed."
    }
    
    # ========== PIN APPS ==========
    Set-TaskbarPin -Unpin * -Silent
    Write-Host "Cleared Pinned Apps."
    Start-Sleep -Seconds 2
    Set-TaskbarPin "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" -Silent
    Write-Host "Pinned Microsoft Edge."
    Start-Sleep -Seconds 1
    Set-TaskbarPin "C:\Program Files\Google\Chrome\Application\chrome.exe" -Silent
    Write-Host "Pinned Google Chome." 
    Start-Sleep -Seconds 1
    ...
    if ($status -eq 'On') {
        ...
        Write-Host "BitLocker enabling started on C Drive."
    }
    ...
    Write-Host "Bitlocker Recovery Key stored on desktop within 'bitlocker-key.txt'."
    ...
    while ($Options) {
        ...
        try {
            ...
            if ($number -ge 1 -and $number -le $Options.Count) {
                & @($Options.GetEnumerator())[$number-1].Value
            } else {
