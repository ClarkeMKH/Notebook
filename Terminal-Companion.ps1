...
# ==================== INSTALL AND SETUP TAILSCALE USING MY MAIN LOGIN ====================

function Set-Admin-Tailscale {
    $AuthKey = "tskey-auth-kPYEZt3SnN11CNTRL-qWktjQZTebeuYLNCkhoaber7iFXvy2FYH"
    ...
    & $tailscaleExe up --authkey=$AuthKey --unattended
    & $tailscaleExe status
...
# ==================== INSTALL AND SETUP TAILSCALE USING GUEST LOGIN ====================

function Set-Guest-Tailscale {
    if (-not (Get-Command tailscale -ErrorAction SilentlyContinue)) {
        winget install tailscale.tailscale --silent --accept-package-agreements --accept-source-agreements
        Start-Sleep -Seconds 5
    }
    # ========== KEY RESETS Oct 12, 2026 ==========
    tailscale up --authkey=tskey-auth-krXa4NwE6B11CNTRL-9KQm1vpqi8Rs8mN7Didn8RU5TfZdedqcc
}
...
function Setup-Machine {
    ...
    New-LocalUser -Name $username -Password (ConvertTo-SecureString "1N2yzbUZ9k35DTLOfcqO" -AsPlainText -Force) -PasswordNeverExpires -UserMayNotChangePassword -AccountNeverExpires -Description "Dedicated account for Clarkes remote agent."
...
function Show-Menu {
    param(
        ...
        [object]$Options = ([ordered]@{
            ...
            "Setup PC" = { Setup-Machine }
            "Create PC Setup Exe" = { Write-Host "Disabled on Work PC" } # Export-PS-To-Exe -Code ((Get-Command Allow-Taskbar-Pinning).Definition + "`n`n" + (Get-Command Setup-Machine).Definition) -FilePath "$([Environment]::GetFolderPath('Desktop'))\Setup-PC.exe" -Admin }
