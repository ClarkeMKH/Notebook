...
function Initialise-Modules {
    ...
    try {
        if (-not (Get-Module -ListAvailable ImagePlayground)) { Install-Module ImagePlayground -Scope CurrentUser -Force }
    ...
    try {
        ...
        $global:Ps2ExeReady = $false
        $global:Ps2ExeError = $_.Exception.Message
    }
}
Initialise-Modules
Initialise-Modules
...
$FullScript = Get-Content -Path $MyInvocation.MyCommand.Path
...
function Sign-In-Microsoft {
    ...
    $panel.Controls.Add((New-Button -Text "Copy URL" -X (($FullWidth/2)-120) -Y (($FullHeight/2)-100) -W 240 -Size 12 -OnClick {
        ...
        [System.Windows.Forms.MessageBox]::Show("URL has been copied.", "Success")
    }))
    $panel.Controls.Add((New-Button -Text "Copy Code" -X (($FullWidth/2)-120) -Y (($FullHeight/2)-30) -W 240 -Size 12 -OnClick {
        ...
        [System.Windows.Forms.MessageBox]::Show("Code has been copied.", "Success")
    }))
...
function Password-Generator {
    ...
    $panel.Controls.Add((New-Button -Text $uniquePassword -X (220 + $offsetX) -Y (12 + $offsetY) -W 200 -H 40 -OnClick { & $copy $uniquePassword }.GetNewClosure()))
    $panel.Controls.Add((New-Label -Text "Short PWPush :" -X (10 + $offsetX) -Y (80 + $offsetY) -Size 20))
    $panel.Controls.Add((New-Button -Text "Click to Copy" -X (220 + $offsetX) -Y (82 + $offsetY) -W 200 -H 40 -OnClick { & $copy $shortPushUrl }.GetNewClosure()))
    $panel.Controls.Add((New-Label -Text "Long PWPush :" -X (10 + $offsetX) -Y (150 + $offsetY) -Size 20))
    $panel.Controls.Add((New-Button -Text "Click to Copy" -X (220 + $offsetX) -Y (152 + $offsetY) -W 200 -H 40 -OnClick { & $copy $longPushUrl }.GetNewClosure()))
}
...
function Show-Notepad {
    $script:ContentPanel.Controls.Clear()
    $script:ContentPanel.Controls.Add((New-Label -Text "Notepad" -X 20 -Y 20 -Size 26))
...
function Build-Companion-EXE {
    ...
    try {
        if (Test-Path $iconPath) {
            Invoke-ps2exe -inputFile $PSCommandPath -outputFile $outputPath -noConsole -iconFile $iconPath
        } else {
            Invoke-ps2exe -inputFile $PSCommandPath -outputFile $outputPath -noConsole
        }
